...
const static int COMMON_SAMPLE_RATE = 16000;
...
MarsChunker *
mars_chunker_new_with_sink (char *input, char *muxer, GstElement *sink)
{
  return g_object_new (MARS_TYPE_CHUNKER,
                       "input", input,
                       "muxer", muxer,
                       "sink", sink,
...
char *
mars_chunker_get_input (MarsChunker *self)
{
  g_assert (MARS_IS_CHUNKER (self));
...
void
mars_chunker_set_maximum_chunk_time (MarsChunker *self, guint64 chunk_time)
{
  g_assert (MARS_IS_CHUNKER (self));

  self->max_chunk_time = chunk_time;
}