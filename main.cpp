...
class Log {
    Level m_level;
public:
    Log(Level l = Level::INFO) { m_level = l; }
                                              ...
                                              const whisper_filters &filters, whisper_mel &mel)
{
    ...
    CHECKPT << n_samples / frame_step + 1 << " , " << mel.n_len << '\n';
...
bool read_pcm32(const std::string &fname, std::vector<float> &pcmf32, std::vector<std::vector<float>> &pcmf32s, bool stereo)
{
...
void featureExtractor(const PCMData& data)
{
    const std::string whisper_filter_fname = "data/mel_filters.bin";

    Log() << "Extracting features from new PCM data...\n";
